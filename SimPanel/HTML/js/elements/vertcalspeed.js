class VerticalSpeed extends SVG.Svg {
    constructor(width, height, num = 5, mul = 100) {
        ...
        var opt = {};
        var font1 = opt.font || { fill: "cyan", 'font-family': 'consolas', 'font-weight': 'bold', 'font-size': 22, leading: '0em' }
        ...
        var r = this.rect(w, 30).move(left, this.Base.y() - 30).fill('#000000');
        this.VsLockText = this.text('-1500').fill('cyan').font(font1);
        this.VsLockText.center(r.cx(), r.cy());
    ...
    setVSLock(vs) {
        ...
        if (this.pVSLock != vs) {
            ...
            this.VsLockText.x(this.width - this.VsLockText.bbox().w -5)
            this.pVSLock = vs
